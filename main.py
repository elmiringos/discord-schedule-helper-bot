...
client = commands.Bot(command_prefix='!', intents = intents) #Создаем тело бота и добавляем префик для наших команд
...
@client.event #Создаем события
async def on_member_join(member): #навзвание "on_member_join" обозначает, что данная функция будет выполняться при каждом заходе нового пользователя на сервер
    role = discord.utils.get(member.guild.roles, id = 961990701741133834) #находим нужную роль
...
async def schedule(ctx, arg):  #Функция для распечатывания расписания. Получает на вход два аргумента,
    ...
    if table == "Выходной": #Делаем проверку на выходной
        ...
        return 0
    if table == "Такой группы не существует":
        await ctx.send("Такой группы не существует")
        return 0
